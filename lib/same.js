 ...
 */
function zeropad (inp, len) {
	inp = inp.toString();
  if (inp.length >= len) {
    return inp;
  }
  
  inp = '0'.repeat(len - inp.length) + inp;
...
SAME.constructMessageByteArray = function(message) {
  ...
  if (message !== null) {
  	...
  	let regionsContent = message.regions.map((region) => {
  		let regionCode = '';
  		
  		regionCode += zeropad(region.subdiv, 1);
  		regionCode += zeropad(region.stateCode, 2);
  		regionCode += zeropad(region.countyCode, 3);
